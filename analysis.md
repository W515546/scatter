...
| **LOC** | ~25-30k Python across `scatter/` (excluding tests) | 2,077 + 2,334 = **4,411** PS1 LOC (two scripts) |
...
| **Parallelism** | ThreadPoolExecutor on step 4 file read/parse (45,260 .cs) | `ForEach-Object -Parallel -ThrottleLimit 8` on csproj parsing (PS7+) |
...
|---|:---:|:---:|
| `<ProjectReference>` | ✅ `graph_builder.py:437-495` | ✅ `Get-ProjectReferences:317-438` |
| `<PackageReference>` (NuGet) | ❌ | ⚠️ parsed but not used for impact (stored for report context) |
| `<Import Project="…props/.targets">` (explicit) | ❌ | ✅ `Get-ProjectReferences` `-IncludeImports` |
| **Implicit `Directory.Build.props/.targets`** | ❌ (no scan for these files anywhere in `scatter/`) | ✅ `Find-NearestFileInAncestors` walks up from each project dir, attaches nearest to `PropsTargetsMap` (lines ~548-620) |
| `namespace_usage` (derived: A uses namespace matching B's) | ✅ `graph_builder.py:256-285` | ❌ |
| `type_usage` (derived: A declares T, B references T) | ✅ `graph_builder.py:287-372` with per-file scope gate | ❌ (but `Analyze-Real*` does a narrower variant for config DI) |
...
- **Test-project exclusion**: `Is-TestProjectPath` — test projects don't count toward blast radius
...
4. **Performance at scale**: Scatter processes 1,587 C# projects + 45,260 source files + 274 MB of content in **~6m cold, ~168s internal**. CDH benchmarks in `Test-WorstCasePerformance.ps1` target 1,000-project repos with a ~30% parallel speedup; warm-cache hits save ~2m — but the cold path is also the common case in practice because almost any PR touches a `.csproj`.
...
1. **MSBuild implicit imports** — the single biggest correctness gap in scatter. CDH indexes every `Directory.Build.props`/`Directory.Build.targets`, walks each project dir upward to find the nearest, and records it in `PropsTargetsMap`. When one changes, it knows every project that transitively consumes it. Scatter would miss this entirely.
...
3. **File-type coverage** — CDH classifies 25+ file extensions into gating categories. Scatter only ingests `.cs/.csproj/.sln`. A PR that changes only `.cshtml`, `.razor`, `.json`, `.xml`, `.resx`, images, or `.rdl` produces zero nodes/edges in scatter.
...
6. **Pipeline mapping layer** — `pipeline-mappings.json` + YAML-derived artifact patterns → project → pipeline mapping. Scatter has a `pipeline_to_app_mapping.csv` example but no equivalent YAML introspection.
7. **Method-level impact (Analyze-Real*)** — fetches the unified diff and extracts changed method names, then uses Roslyn to resolve references. Scatter is file/project-level only.
...
9. **Test-project awareness** — `Is-TestProjectPath` deliberately excludes test projects from the blast-radius count. Scatter counts everything equally.
...
|---|---|---|---|---|
| 1 | No `Directory.Build.props/.targets` indexing | **High** | S | Add a scanner that mirrors `Find-NearestFileInAncestors`: one pass over discovered files (which we now have via unified walk), then per-project attach nearest ancestor. New edge type `msbuild_import` or extend `project_reference`. |
| 2 | No `<Import>` parsing in `parse_csproj_all_references` | **High** | S | Already in the XML tree — add `root.findall(".//Import")` alongside existing ProjectReference handling, filter SDK imports. |
...
| 8 | No method-level impact | Low | L | Would need Roslyn or tree-sitter C# method boundary detection + git-patch hunk parsing. `parsers/ts_queries.py` is the starting point. |
...
1. **`Find-NearestFileInAncestors` → `_build_msbuild_import_index`**. Direct translation. Use the existing unified-walk results; filter to `Directory.Build.props/.targets`; build `dir → props_file` index; walk each project's ancestors. Adds `msbuild_import` edges or a `shared_build_files` node attribute. Single file, <100 lines.
2. **Explicit `<Import>` parsing in `project_scanner.parse_csproj_all_references`**. Already halfway there — XML tree is parsed; add `root.findall(".//Import")` + the SDK-filter regex. <20 lines.
...
4. **Test-project filter**. `Is-TestProjectPath` regex is directly portable; applies to blast-radius counts in `consumer_analyzer`.
...
The hybrid also means scatter gets pushed on the gaps it has (implicit imports, soft refs, .rptproj) — because the CDH layer consuming the graph would be broken without them, which is the right forcing function.

## 10. Bottom line

- **Not redundant; complementary.** They address different ends of the pipeline: scatter is the better indexer (richer edges, sounder caching, better performance per LOC, more extensible), CDH is the better policy layer (change-type classification, pipeline gating, real MSBuild coverage, soft refs).
- **Scatter's real correctness gaps are narrow and tractable** — implicit `Directory.Build.*` imports, explicit `<Import>` parsing, `.rptproj`, and soft-ref scanning are each small, bounded additions.
